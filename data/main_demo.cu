 ...
 * This function initialize the given random state
*/
__global__ void	create_world(hittable **d_list, hittable **d_world, camera **d_camera, curandState *rand_state, float x, float y)
{
 ...
 * by using ppm format
*/
inline void	print(vec3 *buf, std::ostream &out)
{
	out << "P3\n" << W << " " << H << "\n255\n";
	for (int y = H-1; y >= 0; --y)
	{
		for (int x = 0; x < W; ++x)
			write_color(out, buf[W*y + x]);
	}
}
...
int	main(void)
{
	...
	for (float angle = 0.0f; angle < 360.0f; angle += 0.5f) {
		...
		create_world<<<1,1>>>((hittable **)d_list, (hittable **)d_world, d_camera, d_rand_state2, x, y);
		CHECK(cudaGetLastError());
		CHECK(cudaDeviceSynchronize());
		...
		std::ofstream output("ppms/" + std::to_string(angle) + ".ppm");
		#if MANAGED
			print(d_buf, output);
		#else
